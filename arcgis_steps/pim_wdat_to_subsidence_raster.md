...
本流程把沉陷预计软件已经计算完成的 `w.dat`、`w.txt` 或 CSV 转换为沉陷点、分析栅格、沉陷边界、等值线和沉陷云图，并另行生成与 PLUS 主网格对齐的驱动栅格。
...
`CGCS2000_3_Degree_GK_Zone_39` 只适用于数据确属 CGCS2000 3°带第 39 带的情况，不能作为其他矿区默认值。
...
当原始 W 的单位为 mm 且“下沉为负”时，`W / -1000` 对应 `DEPTH_M`。不要直接覆盖原始 W，也不要无条件使用绝对值；若外部软件采用“下沉为正”，应按其说明调整。
...
像元大小由原始点间距和研究目的共同决定，不能固定套用单一数值。
...
采样距离属于制图分辨率，应结合原始点间距、地图比例尺和计算量选择；平滑云图的采样距离不等同于沉陷观测精度。

输出示例：`subsidence_surface_cartographic.tif`。该表面是由外部 W 值重建的制图产品，不是新的 PIM 预测结果。

## 7. 提取沉陷边界

可使用 `Contour List` 提取接近零下沉的边界。将最小有效下沉阈值设为可解释参数，并与字段符号保持一致：
...
分析版本保持边界外为 NoData。将 NoData 设为 0 虽可满足部分显示需求，却会混淆“无数据”和“零下沉”；如需该显示方式，应另存 `_display.tif`，不要覆盖分析栅格。

## 9. 绘制沉陷等值线

可使用 `Contour` 或 `Contour with Barriers`：
...
等值线间距和显式阈值由数据范围、地图比例尺及下沉符号约定确定。只有使用负值 `W_M` 栅格时才使用负阈值；若使用 `DEPTH_M`，应改为正值。等值线只是对已有沉陷表面的制图表达。
...
- 水生植被面积可使用本地水深阈值推定，也可使用实测或解译得到的 `aquatic_vegetation_mask`；
...
沉陷云图、等值线和沉陷积水碳储结果可通过 `compose_layout` 组成专题图。将分析栅格、边界和等值线作为结果图层加入布局副本，并为连续深度图、边界线和碳储结果分别指定 `.lyrx` 符号文件。范围可由沉陷边界或沉陷积水边界图层控制；导出后检查布局验证 JSON，再打开 PDF 或 PNG 人工核对图例、色带和注记。详细字段见 `arcgis_steps/cartographic_outputs.md`。
