...
已检查的 PLUS V1.4.1 boxed 发行包没有公开、可验证的预测 CLI。`scripts/plus_v141_bridge.py` 因而采用本地 GUI 交接：只用空参数启动 `PLUS v1.4.1_boxed.exe`，为 ND、UD、EP、RE 各自写入交接清单和状态文件，并等待用户或本地 Computer Use 完成界面操作。输出写入合同路径后，重跑工作流即可自动接管；`waiting_interactive` 不表示情景预测已完成。
