...
description: 通过 MCP 与可注册软件桥接器直接执行矿区遥感分类、ArcGIS 栅格处理、PLUS 情景模拟、InVEST 碳储量计算和结果验证。用于用户提供空间数据与目标后，由智能体调用本地、局域网或云端的 GEE、ArcGIS Pro、ENVI、PLUS、InVEST、QGIS 或开放 GIS 后端，跟踪任务并返回真实产物；不依赖固定软件安装路径，也不是只给操作教程。
---
...
1. 枚举用户提供的文件，识别格式、年份、矿区类型、目标产物和数据依赖。
2. 调用矿区 GIS MCP 的 `list_backends`，对目标后端调用 `backend_capabilities`。后端可来自桌面软件插件、HTTP 服务、socket 桥接器或本地命令适配器。
3. 根据任务调用明确工具：GEE 用 `run_gee_export`，ENVI 用 `run_envi_classification`，PLUS 用 `run_plus_scenario`，ArcGIS 用 `run_arcgis_operations`，InVEST 用 `run_invest_carbon`。
