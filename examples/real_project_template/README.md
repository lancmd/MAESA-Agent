...
python ..\MAESA-Skill\scripts\project_validator.py --project .\project.json
python ..\MAESA-Skill\scripts\project_workflow.py --project .\project.json --run
```
...
先运行 `..\MAESA-Skill\scripts\maesa_doctor.ps1 -Project .\project.json`。若本机执行策略阻止脚本，使用 `powershell -ExecutionPolicy Bypass -File ..\MAESA-Skill\scripts\maesa_doctor.ps1 -Project .\project.json`。随后运行项目校验。校验只检查路径和配置；工作流中的 `classification_input_preflight` 会在 ENVI 打开前检查 ROI 字段、类别数、几何、CRS 与逐类验证样本数。
