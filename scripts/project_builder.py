...
def build(project_file: Path, project_id: str, workspace: str, imagery_periods: list[dict[str, Any]],
          driver_factors: dict[str, str], mine_boundary: str, carbon_density: str, *, w_dat: str | None = None,
          model_package: str | None = None, training_roi: str | None = None, scheme: str = "high_water_coal_7class",
          w_dat_unit: str | None = None, w_dat_convention: str | None = None) -> dict[str, Any]:
    ...
    roots = sorted({str(Path(value).expanduser().resolve().parent) for value in [
        *[item["path"] for item in normalized_periods], *driver_factors.values(), mine_boundary, carbon_density,
        w_dat, model_package, training_roi] if value})
    ...
    payload: dict[str, Any] = {
        ...
        "inputs": {"imagery_periods": normalized_periods, "imagery": [], "mine_boundary": _local(mine_boundary),
                   ...
                   "subsidence_depth_raster": None, "dem": _local(driver_factors.get("dem")), "driver_factors": {key: _local(value) for key, value in driver_factors.items()}},
    ...
    return {"status": "completed", "project_file": str(project_file), "project_id": project_id,
            ...
            "pending_inputs": (["w_dat_unit_and_convention"] if w_dat and (not w_dat_unit or not w_dat_convention) else []) +
