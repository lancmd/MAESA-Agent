...
startup = (ROOT / "scripts" / "start_skill_mcp.ps1").read_text(encoding="utf-8-sig")
...
process = subprocess.run(["powershell", "-ExecutionPolicy", "Bypass", "-File", str(doctor),
                          "-Python", sys.executable, "-Project", str(ROOT / "examples" / "synthetic_multiperiod" / "project.json")],
                         stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8", errors="replace", check=False)
assert process.returncode == 0, process.stdout
for label in ("MAESA Skill Doctor", "Python 3.11+", "MCP package", "Software:", "GPU:", "Data:", "LULC input"):
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
    ...
    process = subprocess.run(["powershell", "-ExecutionPolicy", "Bypass", "-File", str(ROOT / "scripts" / "start_skill_mcp.ps1"),
